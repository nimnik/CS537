        ...
        const u_char *packet)
{
   ...
   printf("source ip: %s, destination ip: %s, source tcp port: %u, destination tcp port: %u\n", src, dst, tcp_src, tcp_dst);
...
int main() {
  ...
  char filter_exp[] = "tcp dst portrange 10-100"; // Capturing TCP packets with Destination Port between 10 to 100
  bpf_u_int32 net;
